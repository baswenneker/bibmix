...
module Bib2
	...
	class Reference < Bib2::AbstractReference
		...
		def id
			if !@intrahash
				raise Bib2::Error, 'No intrahash identifier present'
			end
			@intrahash
		end
		...
		def tags=(value)
			raise Bib2::ReferenceError.new("Tags have invalid type (#{value.class})") if value.class != Hash
			@tags = value
		end
		...
		def author=(value)
			...
			raise Bib2::ReferenceError.new("Author has an invalid type (#{value.class}, #{value})") if value.class != Array
			@author = value
		end
